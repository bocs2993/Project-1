...
for (cat in cats){
  if (is.factor(cvrdata[,cat])){
    ...
    for (lev in levels(cvrdata[,cat])){
      comparison <- rbind(comparison, c(paste("--",lev ), 
                      paste(s <- sum(normData[,cat]==lev, na.rm=T), " (", round(s*100/ms),
                            "%)", sep=""),
                      paste(s <- sum(mciData[,cat]==lev, na.rm=T), " (", round(s*100/ns),
                            "%)", sep=""),
...
colnames(comparison) <- c("Variable","Normal", "MCI", "Ambiguous At-Risk")
kable(comparison, width=3, 
