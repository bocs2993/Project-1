...
label(cvrdata$systolic.prewaking.surge) <- "Diff. in SBP, Post-Wake Mean - Pre-Wake Mean"
label(cvrdata$systolic.rising.surge) <- "Diff. in SBP, Post-Wake.1 - Pre-Wake.1"
label(cvrdata$nocturnal.systolic.diff.sleep.self.reported) <- "Diff. in SBP, Wake - Sleep"
...
The table below displays all of the descriptive statistics for the patients divided by their diagnosis. Continuous variables have a mean (standard deviation), and discrete variables have a count (percentage). The third column is the p-value for the univariate comparison of the each variable between the different diagnoses (Chi-square test for discrete and Kruskal-Wallis for continuous). A significant p-value (< 0.05) indicates that at least one of the three categories is significantly different than the rest. Each of our predictor variables has a significant p-value. It appears, by looking at the below table, that those diagnosed with MCI have consistently lower systolic differences than the rest of the sample. However, these are all univariate relationships; these trends may not hold in the adjusted analyses.
<<>>=
cats <- cats[-4]
comparison <- c(c(), c(), c(),c())
...
colnames(comparison) <- c("Variable","Normal//N = 101", "MCI//N = 62", 
                          "Ambiguous At-Risk//N = 11", "P-value")
