...
names(cvrdata) <- sub("asl.reac.", "", names(cvrdata), fixed = TRUE)
names(cvrdata) <- sub(".hct", "", names(cvrdata), fixed = TRUE)
names(cvrdata) <- sub(".factor", "", names(cvrdata), fixed = TRUE)
names(cvrdata) <- sub("turnal.systolic.diff.sleep.self.reported", ".sys.diff", 
                      names(cvrdata), fixed = TRUE)
...
names(cvrdata$noc.sys.diff)
@

Comparison of included/excluded data
<<>>=
#######################################
#  Inclusion/Exclusion Comparison     #
#######################################

cats <- names(cvrdata)[2:18]
