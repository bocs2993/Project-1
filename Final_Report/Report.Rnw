...
<<regressionTables, echo=F>>=
for (i in 1:14){
  coef <- mod.noc.sys.diff[[i]]$coefficients
  se <- sqrt(diag(mod.noc.sys.diff[[i]]$var))
  ...
  results <- data.frame(Coefficient = coef, StdError = se, ConfInt=confInt)
